...
export async function createAuction({ profile, onProfileChange }) {
  ...
  const handleSubmit = () => submitPlayerBid(); const handleNext = () => { if (roundClosed) controller.start(controller.getRound() === 5 ? 6 : controller.getRound() + 1); }; const focusBidInput = () => { if (!bidInput.disabled) bidInput.focus(); };
