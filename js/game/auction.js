...
export async function createAuction({ profile, onProfileChange }) {
  ...
  let warehouse; let bidders; let history; let clueHistory; let aiTimers = []; let roundClosed = false; let ended = false; let disposed = false;
  ...
  function setBidControls(enabled, minimum = 0) { bidInput.disabled = !enabled; submitButton.disabled = !enabled; bidInput.value = String(minimum || 0); document.querySelector('#bid-minimum').textContent = minimum ? `本回合最低出價：${format(minimum)}` : '可輸入 0 放棄競標'; }
  ...
  function submitPlayerBid(forcedAmount) {
    if (roundClosed || ended || player().confirmed) return; const round = controller.getRound(); const minimum = round === 6 ? fifthBidFor('player') : 0; const validation = validatePlayerBid(forcedAmount ?? bidInput.value, { money: profile.money, minimum }); if (!validation.valid) { status.textContent = validation.message; return; }
  ...
  const handleSubmit = () => submitPlayerBid(); const handleNext = () => { if (roundClosed) controller.start(controller.getRound() === 5 ? 6 : controller.getRound() + 1); }; const focusBidInput = () => { if (!bidInput.disabled) {
    window.requestAnimationFrame(() => bidInput.focus());
  } };
  const handleReset = () => startWarehouse(); document.querySelector('.bid-input-wrap').addEventListener('click', focusBidInput); submitButton.addEventListener('click', handleSubmit); nextButton.addEventListener('click', handleNext); newWarehouseButton.addEventListener('click', startWarehouse); resetButton.addEventListener('click', handleReset); catalogButton.addEventListener('click', () => openMiniCatalog()); startWarehouse();
  return { leave: () => { if (!roundClosed && !ended) submitPlayerBid(0); }, destroy: () => { disposed = true; clearAiTimers(); document.querySelector('#auction-catalog-modal')?.remove(); controller.destroy(); document.querySelector('.bid-input-wrap').removeEventListener('click', focusBidInput); submitButton.removeEventListener('click', handleSubmit); nextButton.removeEventListener('click', handleNext); newWarehouseButton.removeEventListener('click', startWarehouse); resetButton.removeEventListener('click', handleReset); } };
}
