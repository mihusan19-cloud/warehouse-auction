...
import { revealBonusClue, revealClue, renderClue } from './clue.js';
...
export async function createAuction({ profile, onProfileChange }) {
  const [warehouseResponse, aiResponse, conditionResponse, catalog] = await Promise.all([fetch('data/warehouseTemplates.json'), fetch('data/ai.json'), fetch('data/auctionConditions.json'), loadCatalog()]);
  if (!warehouseResponse.ok || !aiResponse.ok || !conditionResponse.ok) throw new Error('無法載入競標資料。');
  const [template, aiDatabase, conditionConfig] = await Promise.all([warehouseResponse.json(), aiResponse.json(), conditionResponse.json()]);
  ...
  let warehouse; let bidders; let history; let clueHistory; let condition; let aiTimers = []; let bidDraft = '0'; let roundClosed = false; let ended = false; let disposed = false;
  ...
  const chooseCondition = () => { const total = conditionConfig.conditions.reduce((sum, entry) => sum + entry.weight, 0); let roll = Math.random() * total; return conditionConfig.conditions.find((entry) => { roll -= entry.weight; return roll < 0; }) ?? conditionConfig.conditions[0]; };
  const candidateItems = (item) => catalog.filter((candidate) => (!item.knowledge.category || candidate.series === item.series) && (!item.knowledge.size || (candidate.width === item.width && candidate.height === item.height)) && (!item.knowledge.quality || candidate.quality === item.quality));
  ...
  function renderValuation() { const totals = warehouse.items.reduce((result, item) => { if (item.knowledge.value) return { lower: result.lower + item.value, upper: result.upper + item.value }; const values = candidateItems(item).map((candidate) => candidate.value); return { lower: result.lower + Math.min(...values), upper: result.upper + Math.max(...values) }; }, { lower: 0, upper: 0 }); document.querySelector('#warehouse-lower-value').textContent = format(totals.lower); document.querySelector('#warehouse-value-range').textContent = `推測區間 ${format(totals.lower)} ～ ${format(totals.upper)}`; }
  function applyCondition() { if (condition.effect !== 'qualityBroadcast') return; const qualities = [...new Set(warehouse.items.map((item) => item.quality))]; const quality = qualities[Math.floor(Math.random() * qualities.length)]; warehouse.items.filter((item) => item.quality === quality).forEach((item) => { item.knowledge.quality = true; }); condition = { ...condition, description: condition.description.replace('{quality}', quality) }; }

  function renderBidders() {
    document.querySelector('#bidders-list').replaceChildren(...bidders.map((bidder) => { const row = document.createElement('div'); const rank = 1 + bidders.filter((other) => other.lastBid > bidder.lastBid).length; const completedBid = bidder.lastBid === 0 ? '放棄本回合' : condition.effect === 'rankOnly' ? `第 ${rank} 名` : format(bidder.lastBid); const bidText = bidder.lastBid === null ? '等待出價' : roundClosed ? completedBid : '已完成喊價'; const showDialogue = player().confirmed && bidder.dialogue; row.className = `bidder-row${bidder.bidderId === 'player' ? ' is-player' : ''}`; row.innerHTML = `<span class="bidder-avatar">${bidder.avatar}</span><span class="bidder-name"><strong>${bidder.name}</strong></span><span class="bidder-bid">${bidText}</span>${showDialogue ? `<em>${bidder.dialogue}</em>` : ''}`; return row; }));
  }
  ...
  function beginRound(round) {
    ...
    if (clue.type !== 'none') clueHistory.push(clue); if (condition.effect === 'bonusClue' && (round === 1 || round === 3)) { const previousIds = clueHistory.flatMap((entry) => entry.items?.map((item) => item.id) ?? []); clueHistory.push(revealBonusClue(warehouse, previousIds)); } renderClue(clue); renderClueHistory(); renderWarehouse(warehouse, openMiniCatalog); renderCondition(); renderValuation(); renderBidders(); renderHistory(); setBidControls(true, round === 6 ? fifthBidFor('player') : 0, { resetValue: true }); nextButton.hidden = true; newWarehouseButton.hidden = true; status.textContent = round === 6 ? '平手決勝：請提交不低於第五回合的出價。' : '查看情報後，提交本回合唯一出價。'; scheduleAiBids(round);
  }
  ...
  function startWarehouse() { clearAiTimers(); warehouse = generateWarehouse({ ...template, prototypeItems: catalog }); condition = chooseCondition(); applyCondition(); bidders = [{ bidderId: 'player', name: profile.name, avatar: 'P', money: profile.money, lastBid: null, confirmed: false, dialogue: '' }, ...createAiBidders(aiDatabase).map((ai) => ({ ...ai, confirmed: false }))]; history = []; clueHistory = []; ended = false; profile.stats.auctions += 1; onProfileChange(profile); playSound('reveal'); playGameAnimation('reveal', '新倉庫開啟'); controller.start(1); }
