...
export async function createAuction({ profile, onProfileChange }) {
  ...
  function useInstrument(id) {
    if (player().confirmed || clueRevealPending || instrumentUsed || roundClosed || ended || !(profile.auction.instruments[id] > 0)) return;
    const instrument = auctionMeta.instruments.find((entry) => entry.id === id);
    if (!instrument) return;
    profile.auction.instruments[id] -= 1; instrumentUsed = true; clueRevealPending = true;
    const clue = revealInstrumentClue(warehouse, instrument.effect);
  ...
  const handleReset = () => { clueAnimationToken += 1; document.querySelector('#clue-fx')?.remove(); clearAiTimers(); clearAiDeadline(); clearSettlementTimer(); clearRoundSafety(); controller.stop(); ended = true; setMode('lobby'); renderLobby(); }; const handleUseInstrument = (event) => { const id = event.target.closest('[data-use-instrument]')?.dataset.useInstrument; if (id) useInstrument(id); };
