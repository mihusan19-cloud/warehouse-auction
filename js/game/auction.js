...
export async function createAuction({ profile, onProfileChange }) {
  ...
  let warehouse; let bidders = []; let history; let clueHistory; let condition; let instrumentUsed = false; let starting = false; let aiTimers = []; let aiDueMarks = new Map(); let revealTimers = []; let roundSafetyTimer = null; let aiDeadlineTimer = null; let settlementTimer = null; let bidDraft = '0'; let roundClosed = false; let ended = false; let disposed = false;
  ...
  function beginRound(round) {
    clearAiTimers(); clearAiDeadline(); clearRevealTimers(); clearSettlementTimer(); clearRoundSafety(); ensureBidders(); roundClosed = false; instrumentUsed = false; bidders.forEach((bidder) => { bidder.lastBid = null; bidder.confirmed = false; bidder.revealed = false; bidder.dialogue = ''; });
  ...
  const handleReset = () => { clearAiTimers(); clearAiDeadline(); clearSettlementTimer(); clearRoundSafety(); controller.stop(); ended = true; setMode('lobby'); renderLobby(); }; const handleUseInstrument = (event) => { const id = event.target.closest('[data-use-instrument]')?.dataset.useInstrument; if (id) useInstrument(id); };
